...
let TestsHelper = class TestsHelper {
    constructor(flatteningVisitor) {
        this.flatteningVisitor = flatteningVisitor;
    }
    parseProviderName(product) {
        switch (product) {
            case types_1.Product.nosetest: return 'nosetest';
            case types_1.Product.pytest: return 'pytest';
            case types_1.Product.unittest: return 'unittest';
            default: {
...
TestsHelper = __decorate([
    ...
    __param(0, inversify_1.inject(types_2.ITestVisitor)), __param(0, inversify_1.named('TestFlatteningVisitor'))
], TestsHelper);
